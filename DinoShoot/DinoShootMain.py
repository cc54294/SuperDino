...
class Restart:
    ...
    def draw(self, cost_time, is_win):
        ...
        text = self.font.render("游戏时长: %.2f 秒" % cost_time, True, (0, 0, 0))
        text_rect = text.get_rect()
        text_rect.center = (SCREEN_WIDTH//2, SCREEN_HEIGHT//3+50)
        SCREEN.blit(text, text_rect)

        text = self.font.render("按回车键重新游戏", True, (0, 0, 0))
        text_rect = text.get_rect()
...
class DinoShootMain:
    ...
    def start(self):
        ...
        self.back_ground_music = 0  # 0 表示无音乐，1 表示游戏中的音乐，2 表示游戏结束音乐
        self.game_over = 0 # 0 游戏中 1 输了 2 赢了
